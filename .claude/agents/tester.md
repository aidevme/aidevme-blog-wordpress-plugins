...
4. Separately run the `wordpress-plugin-security-checklist` skill over the whole plugin (capability checks, nonce pairs, output escaping, input sanitization, `$wpdb` writes), even for areas the spec does not mention. If it is not already in your context, load it with the Skill tool.
...
If a plugin integrates with a Microsoft product or service (Azure, Microsoft Graph, Entra ID/Azure AD, .NET, Windows), use the `microsoft-docs-lookup` skill to verify the code's assumptions about that API's real behavior rather than taking the implementation's own comments at face value — rare in this WordPress-first repo, so only when such an integration is actually present.
