...
Reference: <https://developer.wordpress.org/cli/commands/server/>

## Description

Launches PHP's built-in web server for a specific WordPress installation.

This command runs on the `before_wp_load` hook, just before the WP load process begins. Uses `php -S` to launch a web server serving the WordPress webroot. <http://php.net/manual/en/features.commandline.webserver.php> Importantly, PHP's built-in web server doesn't support `.htaccess` files. If this is a requirement, please use a more advanced web server.
