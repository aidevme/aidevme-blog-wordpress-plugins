...
Note there are no submit buttons in meta boxes. The meta box HTML is included inside the edit screen's form tags, all the post data including meta box values are transferred via POST when the user clicks on the Publish or Update buttons.
...
In the example below we will make a POST request, the response will either be success or failure, this will indicate whether the value of the `wporg_field` is valid.
