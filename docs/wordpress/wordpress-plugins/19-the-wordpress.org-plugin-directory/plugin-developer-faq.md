...
Reference: <https://developer.wordpress.org/plugins/wordpress-org/plugin-developer-faq/>
...
If you're **contacting us about an existing case** we're already handling with you by email (such as an ongoing review or a plugin closure request), **please reply directly to the original email thread** rather than starting a new message.

Please **do not contact** the Plugins Team regarding **account-related issues**, as we are not the appropriate team to assist with them.
...
You will receive an automated email confirming that your submission has been received. Your plugin will then be added to our review queue, and we'll review it as soon as we reach it.

- If we don't find any issues related to security, compatibility, compliance with the guidelines, or overall presentation, your plugin will be approved.
- If we identify any issues, we'll send you an email explaining what needs to be addressed. The subject line will look like: `[WordPress Plugin Directory] Review in Progress: {Your plugin name}`    *Please be sure to check your spam or junk folder if you don't see this email in your inbox, as it may occasionally be filtered there.*
...
If there is an existing plugin with your name or slug, then you'll get a warning on the submission.
...
If we have to change your permalink (slug) we will always email you to explain why. In general, we change your permalink when you have obvious typos or mistakes (*foundre* instead of *founder*, for example) or if there are conflicts with existing trademarks, project names or other plugins. Please make sure you read your review email carefully, as we do explain why we do things.
...
That error is to inform you that you may not begin your Display Name with someone else's trademarked or common term. This is to protect you and the directory from legal issues regarding trademark abuse and to prevent confusion with 3rd party projects and plugins. To correct the issue, you must change the Display Name in your plugin's readme and main PHP files.
...
Just reply to the email right away and let us know. We can transfer ownership for you. If you forget to do this, you can fix it yourself by adding the correct account as a committer and then having that account remove your own.
...
If your plugin review is not complete after three (3) months, we will reject your submission in order to keep the queue maintainable. At any point in time, we have more than 3500 people mid-review, and we figure that 3 months is a pretty reasonable time frame.
...
We do not accept plugins that have no meaningful purpose or provide no practical functionality.

We also do not accept plugins that facilitate illegal activity or encourage harmful behavior. This includes, but is not limited to, black-hat SEO tools, content spinners, fake content creators, scrappers, hate-related plugins, and similar software.
...
Plugins must not download or execute code from external sources or serve just as a tool to install other plugins. Please refer to Guideline 8 for more information.
...
We do not accept plugins that are essentially copies of existing work or that merely duplicate functionality already provided by WordPress Core. Your plugin should introduce new functionality, solve a specific problem, or provide a meaningfully different approach to an existing solution.

Likewise, we generally do not accept plugins whose functionality is already extensively represented in the Plugin Directory by hundreds of comparable alternatives, unless they provide clear, meaningful differentiation or introduce substantial innovation.
...
As soon as you push code to the SVN folders, your plugin will be live. **DO NOT** push code if you're not ready, as there's no 'off' switch except to close the plugin. As closing a plugin is permanent, we recommend you not push code until you're ready to go live.
...
Per the guidelines, [plugins are limited to 5 tags in their readme](https://developer.wordpress.org/plugins/wordpress-org/detailed-plugin-guidelines/#12-public-facing-pages-on-wordpress-org-readmes-must-not-spam). This helps prevent spam and tag-stuffing. Only the first five tags you declare are displayed and used for search; any additional tags are ignored, so adding more won't help you at all.

In addition, tags that are only used by your plugin won't be shown. Tags are intended to help users discover similar plugins, so a tag with no overlap with other plugins provides little to no discovery value. If one of the tags you declared isn't being shown, this is why.
