...
The plugin creates and manages four dedicated custom database tables — no custom post type, no `wp_posts` usage. All four are created via `dbDelta()` in `Credpl_Installer::install()` and versioned via the `credpl_db_version` option (see `credentials-manager-plugin.php` for the full `CREDPL_DB_VERSION` history). All reads/writes for all four tables go through the single shared data-layer class, `Credpl_Data`. This is a summary reference — `SPECIFICATION.md` §4 is the authoritative, most detailed source; keep both in sync when the schema changes.
