...
- **Sections and their cards** (grouping is defined in the TSX's `sections` array; each section card is full width, with an `<h2>` title via a `CardHeader`, and is a plain container — not clickable, no hover effect):
  ...
  | **Miscellaneous** | Microsoft Certifications → All Microsoft Certifications (`PAGE_MS_CERTIFICATIONS`); Microsoft Exams → All Microsoft Exams (`PAGE_MS_EXAMS`); **Skills** → *no destination yet — a non-interactive card* (§10 v77) |
