...
- **Author (Plugins screen):** `AIDevMe`, set via the `Author:` plugin header and shown as a link to `https://aidevme.com` — WordPress turns "By AIDevMe" into a link automatically when the header block also has `Author URI: https://aidevme.com` (§10 v79; the rule is in core's `get_plugin_data()`). The row has exactly one **"View details"** link, and it is WordPress's own: it opens the details popup that `Credpl_Updater::filter_plugins_api()` answers (§6.9). WordPress only shows it once an update check has supplied this plugin's data, so on a site where GitHub hasn't been reachable yet the row has no "View details" until a check succeeds. An earlier version added its own placeholder link (`credpl_plugin_row_meta()`, `href="#"`); once the updater made WordPress show a real one that became a duplicate, and it was removed in v79. No `Plugin URI:` header is set, because core would add a further "Visit plugin site" link to the row.
...
- Publishing on WordPress.org, and updating from a *private* repository or with a GitHub token (§6.9) — the updater reads the public Releases page only.
...
### 4.6 Table: `{$wpdb->prefix}skills`

One row per skill. A standalone table for now: no relationship to `credentials`, `credential_blocks`, or either Microsoft table (§3), not exposed via the `[credential-block]` shortcode. Managed on the "Skills" admin screen (`Credpl_Admin_Skills`, §6.10) through `Credpl_Data` (§6.6), and reached from the **Skills** card on the Credentials Manager landing page (§6.8).

| Column | Type | Null | Default | Key | Description |
| --- | --- | --- | --- | --- | --- |
| `id` | `BIGINT(20) UNSIGNED` | NOT NULL | `AUTO_INCREMENT` | `PRIMARY KEY` | Internal identifier. |
| `guid` | `VARCHAR(36)` | NOT NULL | — | `UNIQUE KEY` | Stable external identifier, a UUID v4 in the canonical 36-character form (e.g. `3f2b8c1e-9d4a-4e7b-8a52-1c6d0f9e7a34`). Generated server-side by `Credpl_Data::insert_skill()` via `wp_generate_uuid4()` unless the caller supplies one, and never changed by `update_skill()` — any `guid` passed to it is ignored. Distinct from `id`, which is only meaningful inside this database. |
| `skill_name` | `VARCHAR(255)` | NOT NULL | `''` | `KEY` (index) | The skill's display name (the "Skill Name" field). |
| `description` | `TEXT` | NULL | `NULL` | — | Free-text description of the skill. Always written explicitly (`''` when empty) by `insert_skill()`, since a `text` column can't carry a MySQL default (§4.5). |

Unlike the other four tables, `skills` has no `created_at`/`updated_at` columns — it was specified with exactly the four columns above. Adding them later is a `dbDelta()`-handled column addition (§4.5) plus a `CREDPL_DB_VERSION` bump.
...
1. `Credpl_Installer::install()` first runs a one-time migration (`maybe_rename_forms_table_to_blocks()`, §10) for sites upgrading from the pre-rename schema, then creates all five tables (§4.5) via `dbDelta()`.
...
6. `uninstall.php` is a no-op by default — all five tables and their rows are left in place when the plugin is deleted, consistent with the earlier versions of this spec. Dropping user data automatically on uninstall is avoided.
...
A top-level **"Credentials Manager"** menu (`dashicons-awards` icon). Clicking the top-level item itself lands on the **Credentials Manager** landing page (§6.8, §10 v72–v75, v77) — a React/Fluent UI 9 page of four full-width section cards (Main, Miscellaneous, Style, Integrations) holding navigation cards — not on any of the lists; before v72 it landed on the Credential Blocks list, mirroring Contact Form 7's "Contact" menu. Six *visible* submenu items:

| Label | Screen |
| --- | --- |
| Credentials Manager | Landing page (§6.8, React/Fluent UI, sectioned navigation cards) — also the top-level menu's own click target |
| All Credential Blocks | Credential Blocks list (React/Fluent UI) |
| All Credentials | Credentials list (React/Fluent UI) |
| All Microsoft Certifications | Microsoft Certifications list (React/Fluent UI) |
| All Microsoft Exams | Microsoft Exams list (React/Fluent UI) |
| All Skills | Skills list (React/Fluent UI, §6.10) |
| Integration | Placeholder screen (§6.7) — no integrations exist yet |

**The five "Add …" screens (Add Credential Block, Add Credential, Add Microsoft Certification, Add Microsoft Exam, Add Skill) are deliberately *not* in this visible list** (§10 v26) — each is still registered normally via `add_submenu_page()`, exactly like every other row in the table above (nothing about their registration differs), so their page slugs stay fully reachable. What hides them from the sidebar is a separate step, `Credpl_Admin_Menu::hide_add_screens_from_menu()` (hooked to `admin_head`, since the WP admin sidebar is global, not scoped to this plugin's own screens): a small inline `<style>` block that targets each hidden page's exact link (`#adminmenu a[href$="page=credpl-credential-new"]{display:none;}`, one rule per page) and hides it visually. **Deliberately not `remove_submenu_page()`** — an earlier version (v26) tried that instead, and it broke direct access to all four pages entirely (v28 fixes this; see that note for the root cause) because removing an entry from the `$submenu` global also breaks WordPress core's own resolution of that page's parent menu, which core's access-control check depends on for *every* request to that page, not just sidebar rendering. Each screen is still reachable three other ways: its list's own **Add New** button, each list row's **Edit** link, and — since a create/update redirects back to the edit screen with `updated=1` (§10 v24/v25) — a **"Back to All …"** button shown right there. With those three entry points already covering every real navigation path to these screens, a fifth path (a permanent, always-visible top-level menu item) was redundant. On all four lists — Credentials, Credential Blocks, Microsoft Certifications, and Microsoft Exams — "Add New" is the toolbar's **New** button (§6.2.3/§6.3.2/§6.4.2/§6.5.2, §10 v56/v59/v60/v61); each screen's PHP-rendered `page-title-action` **Add New** link was removed once its own toolbar covered the same destination, so it's not duplicated two ways on any of them.
...
- `package.json` (plugin-local, `private: true`) with devDependencies `@wordpress/scripts`, `typescript`, `@types/react`, `@types/react-dom`, and `adm-zip` (added in CHANGE_LOG.md v67 — see `package-zip` below; previously only pulled in transitively by `@wordpress/scripts`'s own toolchain, now depended on directly), and eight scripts. `build` (`wp-scripts build`), `start` (`wp-scripts start`, for local watch-mode development), and `check-types` (`tsc --noEmit`, for standalone type-checking — see the TypeScript note below) are the original three. The other five are npm-lifecycle housekeeping layered on top of `build`, added across CHANGE_LOG.md v64–v67: `lint-php` (`node bin/lint-php.js` — runs `php -l` against every `.php` file in the plugin; see `DEVELOPER.md` for installing a PHP CLI on Windows and what this does and doesn't catch), `build:release` (`npm run lint-php && npm run build` — the one command to run before a release; `&&` means a failed lint stops the build from running at all), `prebuild` (`node bin/bump-version.js` — an npm lifecycle hook, not a script anyone runs by name, that npm executes automatically before `build` runs, including the nested `npm run build` inside `build:release`, exactly once either way; increments the patch version in `package.json`'s `"version"` and both places `credentials-manager-plugin.php` states its own version, keeping the two from drifting apart — but, unlike the manual, deliberate version bumps in every CHANGE_LOG.md entry up through v65, it now runs on *every* `npm run build`, including routine development builds, not just once per finished, documented change; see `DEVELOPER.md` for the `npx wp-scripts build` escape hatch when that's not wanted mid-iteration), `postbuild` (`node bin/package-zip.js` — the `prebuild` hook's mirror image, npm-executed automatically right after `build` finishes, same "fires exactly once even through `build:release`'s nested `npm run build`" behavior; rebuilds `dist/credentials-manager-plugin.zip` directly from whatever's currently in `includes/`, `build/`, `assets/`, and the fixed set of shippable root docs — see `package-zip` below), and `package-zip` (`node bin/package-zip.js` — the same script as `postbuild`, exposed as its own named command for repackaging the zip on its own, e.g. after a docs-only edit that doesn't need a rebuild). None of `lint-php`/`build:release`/`prebuild`/`postbuild`/`package-zip` touches anything TypeScript/webpack-related, and none of their own scripts (`bin/lint-php.js`, `bin/bump-version.js`, `bin/package-zip.js`) has a build step of its own — they're here purely because `npm` is already this plugin's one common entry point for "run a check (or a housekeeping step) before committing/packaging." `bump-version.js` deliberately does not touch `CREDPL_DB_VERSION` (schema-change-only, still hand-bumped) or write a CHANGE_LOG.md entry (still a deliberate, hand-authored step for every real change); `package-zip.js` deliberately builds its root-file list from a fixed allowlist (not "everything in the plugin root") so a dev-only file like `package.json` itself never ends up in the zip just by existing alongside the ones that should, while its `includes/`/`build/` file lists are globbed rather than hardcoded, so a new admin class or a new webpack entry needs no edit to this script to start shipping. Regular `dependencies` (bundled into the compiled JS, not just build tooling): `@fluentui/react-components` (Fluent UI 9), `@fluentui/react-datepicker-compat` (the `DatePicker`), and `@fluentui/react-icons` (the icon set — first used by the Credentials list toolbar's icon-only buttons, §6.2.3, §10 v43) — the same category of dependency as `@dnd-kit/*` for the Credential Block picker (§6.3.1), for the same reason: none of the three is a `@wordpress/*` package the dependency-extraction plugin knows how to externalize to a WordPress core global, so all three get bundled into whichever entry imports them (`build/credential.js` ~312 KB minified, up from ~6 KB before this — an accepted trade-off for a richer admin-only form, same reasoning as §6.3.1's bundle-size note).
...
**Mount point**: `Credpl_Admin_Credentials::render_list_page()` renders `<div id="credpl-credentials-list-root"></div>` in place of `Credpl_Credentials_List_Table::display()`'s HTML table; the surrounding chrome (`<h1>`, the save/delete success notices) stays PHP-rendered, exactly the pattern the Add/Edit forms already use for their own mount points (§6.2, §6.3) — except this screen's `<h1>` no longer has its own **Add New** `page-title-action` link next to it (§10 v56); the toolbar's **New** button below covers that.
...
**Toolbar** (§10 v41/v43/v53/v54): a Fluent UI `Toolbar`, wrapped in a Fluent UI `Card` (default `appearance="filled"` — rounded corners, `shadow4`, `colorNeutralBackground1` background, all for free from the component itself, no custom CSS needed for any of the three) rendered above the table — `ToolbarButton`s **Back**, a `ToolbarDivider`, **New**, **Edit**, another `ToolbarDivider`, then **Delete** — reads the very same `selectedIds` state (Back excepted, §10 v74). The `Card` stretches to the full width of the table below it, matching the block-level `<div>` the table sits in (a `Card` is a block element and fills its container's width by default; the `toolbarCard` style rule sets no `width` of its own, §10 v54 — an earlier version constrained it to `width: fit-content` so it hugged just the three buttons, but that read as a small floating panel rather than a toolbar spanning the table it belongs to). Each button is icon-only (`ArrowLeftRegular`/`AddRegular`/`EditRegular`/`DeleteRegular` from `@fluentui/react-icons`, via the `icon` prop) with no visible text label, wrapped in a Fluent UI `Tooltip` (`relationship="label"` — the accessible-name mechanism Fluent's own guidance calls for on an icon-only control with no visible text, same pattern already used for the Description column's truncated-text tooltip, §10 v36) carrying a full sentence explaining what the button does and, since the enablement rules aren't obvious from the icon alone, what makes it available. Disabled buttons use `disabledFocusable` rather than `disabled` specifically so their tooltip still shows on hover/focus while disabled (a plain `disabled` native `<button>` typically can't receive hover events at all) — functionally identical to `disabled` otherwise, since `useARIAButtonProps()` (`@fluentui/react-aria`) strips the `onClick` handler from the rendered button either way, so no separate click-guard is needed in this component's own handlers:

- **Back** (§10 v74) — tooltip *"Go back to the Credentials Manager page."* Always enabled, regardless of selection, and navigates to `config.backUrl` — the Credentials Manager landing page (§6.8, `Credpl_Admin_Menu::PAGE_MANAGER`), which PHP builds and localizes alongside `addNewUrl`. The first button in the toolbar, followed by its own `ToolbarDivider` so it reads as navigation rather than as one of the New/Edit/Delete record actions. Identical on all four list screens (§6.3.2/§6.4.2/§6.5.2 inherit it unchanged).
- **New** — tooltip *"Create a new credential. Deselect all credentials to enable this button."* Disabled whenever anything is selected (`selectedIds.size > 0`) and otherwise navigates to `config.addNewUrl` (the Add New Credential screen) — the *only* way to reach it from this screen now that the Credentials list's own `page-title-action` **Add New** link has been removed (§10 v56), unlike the other three lists, which still have both.
...
**Shared table component** (§10 v83): the table itself — selection column, sortable headers, selected-row highlight, empty state, footer — is `DataTable` (`src/components/tables/table.tsx`), used by all five list screens (Credentials, Credential Blocks, Microsoft Certifications, Microsoft Exams, Skills); each screen supplies its `columns`, its already-translated labels, and its selection state (`useRowSelection()`), and keeps its own confirmation dialogs. Behavior is unchanged from the earlier per-screen copies.

**Shared toolbar component** (§10 v88): the `Card`-wrapped `Toolbar` above the table — Back, New, Edit, Delete, and an optional trailing Sync button — is `ListPagesToolbar` (`src/components/toolbars/list-pages-toolbar.tsx`), used by all five list screens in place of the per-screen copy each one used to carry. Back is hardcoded (identical everywhere); New/Edit/Delete tooltip text stays a screen-supplied prop; New's and Delete's disabled state are booleans the screen computes from its own selection (`selectedIds`); Edit's disabled state is derived from whether an `editUrl` prop (`singleSelectedRow?.editUrl`) was passed, rather than a second boolean that could drift out of sync with it; Delete and the optional `sync.onClick` both call back into the screen rather than navigating directly, since each opens that screen's own `ConfirmationDialog`. The `sync` prop is passed only by the Microsoft Certifications and Microsoft Exams screens. Behavior is unchanged from the earlier per-screen copies.

**Table footer** (§10 v49): a `TableFooter` component (`src/components/tables/table-footer.tsx`) renders directly below the `Table`, showing **"Number of Records: N"** and **"Number of Selected Records: N"** on the same line (a `display: flex` row with a gap between the two `Text`s — no separator character, just spacing). Like `ConfirmationDialog` (§10 v42), it's a small, generic, **not** Credentials-specific component — its only props are `recordCount`/`selectedCount` (both plain `number`s the caller computes; here, `config.rows.length` and `selectedIds.size`), so it has no dependency on what a "record" actually is and can be dropped into any other Fluent UI table screen the same way. Rendered only alongside the table itself (i.e. not shown in the empty-list state, §6.2.3 above, where there's nothing meaningful to count).
...
**List: a React app built on Fluent UI 9's plain `Table` building blocks, not a `WP_List_Table`** (§10 v59 — the second of the plugin's four list tables converted, after Credentials, §6.2.3/§10 v34; see §6.3.2 for the build setup, which mirrors §6.2.3 closely enough that it's documented as a diff against it rather than repeated in full — `REACT-DEVELOPER-GUIDE.md` has the general-purpose write-up of the pattern itself). A `Toolbar` above the table (New / Edit / Delete) and a leading checkbox column let one or more rows be selected at once, same as Credentials (§6.2.3). Columns Title (a Fluent UI `Link` to the block's edit screen), Shortcode (the `[credential-block id="…"]` string, click-to-select, same as Contact Form 7's list — a plain read-only `<input>`, not a Fluent `Input`, since a Fluent component adds no value for a field that's never edited), Description (truncated with an ellipsis and wrapped in a `Tooltip` showing the full text, `—` when not set at all). Title and Description are sortable (clicking a header does a full page reload to `?orderby=…&order=…`, matching what those columns did before this screen was converted from a `WP_List_Table`); Shortcode isn't, since it's a generated identifier rather than descriptive data. The underlying query is unchanged: `Credpl_Data::get_credential_blocks()`, which validates `orderby`/`order` against a fixed column allowlist before they reach SQL (§8) — PHP still runs this query once per page load and hands the resulting rows to React as already-fetched, already-sorted data; there is no client-side re-fetch or re-sort.
...
Delete: `admin-post.php?action=credpl_delete_credential_block&id=…`, nonce-protected — this single-row endpoint is left in place but, since §10 v59, is no longer linked from the list screen's UI; deleting one or more blocks now goes through selecting their rows and the list's toolbar Delete button (§6.3.2), which calls a bulk-delete action instead (same relationship as Credentials' `DELETE_ACTION` vs. `BULK_DELETE_ACTION`, §10 v41/v56).
...
#### 6.3.2 The Credential Blocks list: Fluent UI 9's plain `Table` building blocks

The second list screen converted from `WP_List_Table` to this plugin's Fluent UI `Table` pattern (§10 v59), following the same pattern §6.2.3 documents in full for Credentials and `REACT-DEVELOPER-GUIDE.md` writes up generically — this subsection only calls out what differs.

- **Mount point / source**: `Credpl_Admin_Blocks::render_list_page()` renders `<div id="credpl-credentials-blocks-list-root"></div>`, no `page-title-action` **Add New** link (removed the same way Credentials' was, §10 v56) — the toolbar's **New** button covers it. `src/credentials-blocks-list.tsx` is a sixth `webpack.config.js` entry, compiled to `build/credentials-blocks-list.js`/`.asset.php` (~325 KB minified — no `@dnd-kit`, same category of admin-only bundle-size trade-off as every other Fluent UI-heavy entry in this plugin, §6.2.1/§6.3.1).
- **Data flow**: `Credpl_Admin_Blocks::enqueue_list_assets()` (dispatched from `enqueue_assets()` based on `$_GET['page']`, the same two-page-one-dispatcher shape `Credpl_Admin_Credentials::enqueue_assets()` uses) reads/validates `orderby`/`order`, runs `Credpl_Data::get_credential_blocks()` once, and localizes `window.credplCredentialBlocksList = { rows, listUrl, orderby, order, addNewUrl, bulkDeleteUrl, noItemsText }` — each row is `{ id, title, editUrl, shortcode, description }`, `shortcode` being the same `Credpl_Shortcode::build_tag( $block )` string the Add/Edit screen's own read-only field (§6.3 above) and the old list table's Shortcode column both already used.
- **Shortcode cell**: `CredentialBlockShortcodeCell()` renders a plain, native `readOnly` `<input>` (not a Fluent `Input`) with `onClick={ ( event ) => event.currentTarget.select() }` — clicking it selects the full value for copying, the same UX the PHP-rendered version used (`onclick="this.select();"`).
- **Toolbar / selection / delete confirmation / footer**: identical pattern to Credentials (§6.2.3) — icon-only `Toolbar` wrapped in a `Card`, `TableSelectionCell`-based multi-select, a shared `ConfirmationDialog` instance driven by a `useState<number[] | null>`, and a `TableFooter` below the table. `Credpl_Admin_Blocks::BULK_DELETE_ACTION`/`bulk_delete()` mirror `Credpl_Admin_Credentials`'s own bulk-delete action field-for-field (one fixed nonce action string, `absint()`-sanitized `ids[]`, a `bulk_deleted=<count>` redirect arg distinct from the existing single-row `deleted=1` flag, rendered via `_n()`).
- **Removed**: `Credpl_Blocks_List_Table` (`includes/class-credpl-blocks-list-table.php`) is deleted outright, the same way `Credpl_Credentials_List_Table` was (§10 v34) — nothing else referenced it once this screen stopped calling it. The remaining two list tables (`Credpl_Ms_Certifications_List_Table`, `Credpl_Ms_Exams_List_Table`, §6.4/§6.5) are untouched and still `WP_List_Table`-based.

### 6.4 Microsoft Certifications screen

**List: a React app built on Fluent UI 9's plain `Table` building blocks, not a `WP_List_Table`** (§10 v60 — the third of the plugin's four list tables converted, after Credentials and Credential Blocks, §6.2.3/§6.3.2; see §6.4.2 for the build setup, documented as a diff against §6.3.2). A `Toolbar` above the table (New / Edit / Delete / **Sync Certifications**, §6.4.2) and a leading checkbox column let one or more rows be selected at once, same as Credentials/Credential Blocks. Columns Title (a Fluent UI `Link` to the certification's edit screen), Icon (a small `<img>` from `icon_url` — an external URL, not a media library attachment, so no `wp_enqueue_media()`/picker is needed anywhere on this screen), Certification Type, Type, Last Modified (`date_i18n()` against the site's `date_format` **and** `time_format` options, since `last_modified` is a full datetime, unlike Credentials' date-only `earned_on`/`expires_on`, §6.2). Title, Certification Type, Type, and Last Modified are sortable (clicking a header does a full page reload to `?orderby=…&order=…`, matching what those columns did before this screen was converted from a `WP_List_Table`); Icon isn't, since it's an image, not descriptive data. The underlying query is unchanged: `Credpl_Data::get_ms_certifications()`, which validates `orderby`/`order` against a fixed column allowlist before they reach SQL (§8) — PHP still runs this query once per page load and hands the resulting rows to React as already-fetched, already-sorted data; there is no client-side re-fetch or re-sort.
...
**Sync Certifications**: a toolbar `ToolbarButton` on the list screen (§6.4.2, replacing the earlier `page-title-action` link, §10 v60), nonce-protected (`wp_nonce_url()`/`check_admin_referer()`) and gated behind a `ConfirmationDialog` (the same shared, generic dialog component the Delete confirmation uses, §6.2.3) rather than a native JS `confirm()` — the backend nonce/`check_admin_referer()` protection is identical either way; only how the confirmation prompt itself is rendered changed. `admin-post.php?action=credpl_sync_ms_certifications` → `Credpl_Admin_Ms_Certifications::sync()`:
...
#### 6.4.2 The Microsoft Certifications list: Fluent UI 9's plain `Table` building blocks

The third list screen converted from `WP_List_Table` to this plugin's Fluent UI `Table` pattern (§10 v60), following the same pattern §6.2.3 documents in full for Credentials and §6.3.2 documents as a diff against it for Credential Blocks — this subsection only calls out what differs, including the one addition beyond either of those two screens: a **Sync Certifications** toolbar button.

- **Mount point / source**: `Credpl_Admin_Ms_Certifications::render_list_page()` renders `<div id="credpl-ms-certifications-list-root"></div>`, no `page-title-action` **Add New**/**Sync Certifications** links (both removed — the toolbar's **New**/**Sync Certifications** buttons cover them, §10 v60). `src/ms-certifications-list.tsx` is a seventh `webpack.config.js` entry, compiled to `build/ms-certifications-list.js`/`.asset.php` (~327 KB minified, same category of admin-only bundle-size trade-off as every other Fluent UI-heavy entry in this plugin).
- **Data flow**: `Credpl_Admin_Ms_Certifications::enqueue_list_assets()` (dispatched from `enqueue_assets()` based on `$_GET['page']`, the same two-page-one-dispatcher shape the other three admin classes use) reads/validates `orderby`/`order`, runs `Credpl_Data::get_ms_certifications()` once, and localizes `window.credplMsCertificationsList = { rows, listUrl, orderby, order, addNewUrl, bulkDeleteUrl, syncUrl, noItemsText }` — each row is `{ id, title, editUrl, iconUrl, certificationType, type, lastModifiedDisplay, lastModifiedRaw }`, `lastModifiedDisplay` being the same `date_i18n()`-formatted string the old list table's Last Modified column showed, `lastModifiedRaw` the raw `Y-m-d H:i:s` value (rendered into a `<time dateTime="…">` element for semantic markup).
- **Icon cell**: a plain `<img src={iconUrl} alt="" />` (32×32, `object-fit: contain`, `src/styles/msCertificationsList.styles.ts`'s `iconImg` rule) — same external-URL, no-media-library-picker reasoning as the Add/Edit form's own Icon URL preview (§6.4) — or an em dash when a row has no icon set.
- **Toolbar / selection / delete confirmation / footer**: identical pattern to Credentials/Credential Blocks (§6.2.3/§6.3.2) — icon-only `Toolbar` wrapped in a `Card`, `TableSelectionCell`-based multi-select, a shared `ConfirmationDialog` instance driven by a `useState<number[] | null>`, and a `TableFooter` below the table. `Credpl_Admin_Ms_Certifications::BULK_DELETE_ACTION`/`bulk_delete()` mirror the other two admin classes' own bulk-delete actions field-for-field.
- **Sync Certifications button** (§10 v60): a fourth `ToolbarButton` (`ArrowSyncRegular` icon, plus a visible text label — unlike New/Edit/Delete, which are icon-only, this one keeps a label since it isn't a common CRUD action a user would recognize by icon alone), separated from Delete by a `ToolbarDivider`, always enabled (sync doesn't depend on any row selection). Clicking it opens a second `ConfirmationDialog` instance (its own `useState<boolean>`, `pendingSync`) with the exact confirmation text the old `confirm()` dialog used — *"Sync certifications from Microsoft Learn now? This fetches the full catalog and may take a minute."* — and, on confirm, navigates to `config.syncUrl` (a nonce URL `enqueue_list_assets()` localizes the same way `bulkDeleteUrl` is, just with no `ids[]` to append, since Sync takes no per-row input). This is a plain navigation, not a `fetch`/REST call, matching this plugin's established data-flow convention (§6.2.3) — the backend `sync()` handler (§6.4) is completely unchanged; only how the confirmation prompt is triggered moved from a PHP-rendered link's inline `onclick="confirm(...)"` to this dialog.
- **Removed**: `Credpl_Ms_Certifications_List_Table` (`includes/class-credpl-ms-certifications-list-table.php`) is deleted outright, the same way the other two list tables were (§10 v34/v59) — nothing else referenced it once this screen stopped calling it.

### 6.5 Microsoft Exams screen

**List: a React app built on Fluent UI 9's plain `Table` building blocks, not a `WP_List_Table`** (§10 v61 — the fourth and last of the plugin's four list tables converted; see §6.5.2 for the build setup, documented as a diff against §6.4.2). A `Toolbar` above the table (New / Edit / Delete / **Sync Exams**, §6.5.2) and a leading checkbox column let one or more rows be selected at once, same as the other three lists. Columns Title (a Fluent UI `Link` to the exam's edit screen), Icon (external `icon_url`, same as Microsoft Certifications' Icon column, §6.4), Display Name, Type, Last Modified (`date_i18n()` against `date_format`/`time_format`, same as Microsoft Certifications' Last Modified). Title, Display Name, Type, and Last Modified are sortable (clicking a header does a full page reload to `?orderby=…&order=…`, matching what those columns did before this screen was converted from a `WP_List_Table`); Icon isn't. The underlying query is unchanged: `Credpl_Data::get_ms_exams()`, which validates `orderby`/`order` against a fixed column allowlist before they reach SQL (§8) — PHP still runs this query once per page load and hands the resulting rows to React as already-fetched, already-sorted data; there is no client-side re-fetch or re-sort.
...
**Sync Exams**: same conversion as Microsoft Certifications' **Sync Certifications** (§6.4.2, §10 v60/v61) — a toolbar `ToolbarButton` on the list screen (§6.5.2, replacing the earlier `page-title-action` link), nonce-protected (`wp_nonce_url()`/`check_admin_referer()`) and gated behind a `ConfirmationDialog` rather than a native JS `confirm()`. `admin-post.php?action=credpl_sync_ms_exams` → `Credpl_Admin_Ms_Exams::sync()` fetches the *same* `CATALOG_API_URL` Microsoft Certifications' sync already fetches (`https://learn.microsoft.com/api/catalog/?type=certifications,exams&locale=en-us` — one HTTP request already returns both arrays; each screen's sync just reads its own half), but reads the response's `exams` array instead of `certifications`, maps each entry through the same field-by-field sanitization rules `save()` applies (`map_catalog_exam()`, §8), and upserts by `uid` (`Credpl_Data::get_ms_exam_by_uid()`). Redirects with `synced=1` plus `sync_created`/`sync_updated`/`sync_skipped`, rendered as **"Sync complete: N created, M updated, K skipped."** plus **"First reason: …"** when `K > 0` — this screen shipped with the skip-diagnostics behavior from the start (§10 v23), rather than needing a v21-style follow-up fix the way Microsoft Certifications did.

#### 6.5.2 The Microsoft Exams list: Fluent UI 9's plain `Table` building blocks

The fourth and last list screen converted from `WP_List_Table` to this plugin's Fluent UI `Table` pattern (§10 v61), following the same pattern §6.4.2 documents for Microsoft Certifications — this subsection only calls out what differs, which is almost nothing beyond field names.

- **Mount point / source**: `Credpl_Admin_Ms_Exams::render_list_page()` renders `<div id="credpl-ms-exams-list-root"></div>`, no `page-title-action` **Add New**/**Sync Exams** links (both removed — the toolbar's **New**/**Sync Exams** buttons cover them, §10 v61). `src/ms-exams-list.tsx` is an eighth `webpack.config.js` entry, compiled to `build/ms-exams-list.js`/`.asset.php` (~327 KB minified, same category of admin-only bundle-size trade-off as every other Fluent UI-heavy entry in this plugin).
- **Data flow**: `Credpl_Admin_Ms_Exams::enqueue_list_assets()` (dispatched from `enqueue_assets()` based on `$_GET['page']`) reads/validates `orderby`/`order`, runs `Credpl_Data::get_ms_exams()` once, and localizes `window.credplMsExamsList = { rows, listUrl, orderby, order, addNewUrl, bulkDeleteUrl, syncUrl, noItemsText }` — each row is `{ id, title, editUrl, iconUrl, displayName, type, lastModifiedDisplay, lastModifiedRaw }` (`displayName` standing in for Microsoft Certifications' `certificationType` — the two tables' one differing sortable column, §4.3/§4.4).
- **Icon cell**: identical to Microsoft Certifications (`src/styles/msExamsList.styles.ts`'s own `iconImg` rule, same 32×32/`object-fit: contain` values).
- **Toolbar / selection / delete confirmation / footer**: identical pattern to the other three lists (§6.2.3/§6.3.2/§6.4.2). `Credpl_Admin_Ms_Exams::BULK_DELETE_ACTION`/`bulk_delete()` mirror the other three admin classes' own bulk-delete actions field-for-field.
- **Sync Exams button** (§10 v61): identical to Microsoft Certifications' Sync Certifications button (§6.4.2), with its own `ConfirmationDialog` confirmation text — *"Sync exams from Microsoft Learn now? This fetches the full catalog and may take a minute."* — navigating to `config.syncUrl` on confirm. The backend `sync()` handler (§6.5) is completely unchanged; only how the confirmation prompt is triggered moved.
- **Removed**: `Credpl_Ms_Exams_List_Table` (`includes/class-credpl-ms-exams-list-table.php`) is deleted outright, the same way the other three list tables were (§10 v34/v59/v60) — nothing else referenced it once this screen stopped calling it.

### 6.6 Shared data layer

`Credpl_Data` is the single place that talks to `$wpdb` for all five tables — every admin screen and the shortcode renderer (§7) go through it, so there's one place query shape, escaping, and sanitization live:
...
- `get_skills( array $args = array() ): array`
- `get_skill( int $id ): array|null`
- `get_skill_by_guid( string $guid ): array|null`
- `insert_skill( array $data ): int|WP_Error`
- `update_skill( int $id, array $data ): bool|WP_Error`
- `delete_skill( int $id ): bool`
...
### 6.8 Credentials Manager landing page

The top-level menu item's own click target (§6.1, §10 v72–v75, v77), page slug `Credpl_Admin_Menu::PAGE_MANAGER` (`credpl-credentials-manager`). A React app on Fluent UI 9, built like the list/form screens (§6.2.1): four full-width **section cards**, each holding the clickable **navigation cards** for that group.

- **Mount point / PHP**: `Credpl_Admin_Manager::render_page()` renders the `<h1>Credentials Manager</h1>` in plain PHP (same chrome convention as every other screen) and a single empty `<div id="credpl-credentials-manager-page-root"></div>`, behind the same `manage_options` capability check as every other screen.
- **Source**: `src/credentials-manager-page.tsx`, the ninth webpack entry (`build/credentials-manager-page.js` + `.asset.php`), renders `<FluentProvider theme={ webLightTheme }>` around a vertical stack of section `Card`s. Styles live in `src/styles/credentialsManagerPage.styles.ts` (`useCredentialsManagerPageStyles`).
- **Sections and their cards** (grouping is defined in the TSX's `sections` array; each section card is full width, with an `<h2>` title via a `CardHeader`, and is a plain container — not clickable, no hover effect):

  | Section | Navigation cards → destination |
  | --- | --- |
  | **Main** | Credential Blocks → All Credential Blocks (`PAGE_BLOCKS`); Credentials → All Credentials (`PAGE_CREDENTIALS`) |
  | **Miscellaneous** | Microsoft Certifications → All Microsoft Certifications (`PAGE_MS_CERTIFICATIONS`); Microsoft Exams → All Microsoft Exams (`PAGE_MS_EXAMS`); Skills → All Skills (`PAGE_SKILLS`, §6.10; the card had no destination until §10 v81) |
  | **Style** | *(none yet)* — shows a muted "Nothing here yet." line |
  | **Integrations** | Integrations → the Integration screen (`PAGE_INTEGRATION`, §6.7) |

  Each section card is named by its heading (`aria-labelledby`). The navigation cards sit in a CSS grid inside their section (`repeat( auto-fill, minmax( min( 280px, 100% ), 1fr ) )`): `auto-fill`, not `auto-fit`, so a section with a single card (Integrations) doesn't stretch it across the whole row; the 280px minimum keeps "Microsoft Certifications" on one line; the `min( …, 100% )` guard means a very narrow container (checked at 360px) stacks the cards full width instead of overflowing.
- **Navigation cards**: each is an `appearance="outline"` Fluent `Card` — an icon (`@fluentui/react-icons`, brand-coloured) plus the label — nested inside its section card, with no other content.
- **Hover effect** (navigation cards that have a destination): the card lifts 4px (`translateY( -4px )`), gains `shadow16`, and its border turns `colorBrandStroke1`, animated over `durationNormal`; the cursor is a pointer. Fluent draws a `Card`'s border on its `::after` pseudo-element, not on the element itself, so the recolour targets `:hover::after`. Under `prefers-reduced-motion: reduce` the transition is effectively instant and the lift is dropped (the shadow/border change remains).
- **Cards without a destination** (§10 v77): a card whose definition in the TSX's `sections` array has no `urlKey` — none currently; **Skills** was the only one until §10 v81 gave it a destination — is a plain, non-interactive `Card`: no `Link`, no click handler, no tab stop, and none of the pointer cursor, hover lift, shadow, or brand-coloured border above, so it can't be mistaken for a button that does nothing (its title is ordinary text, not link-coloured, which is also what tells it apart at a glance). It has the same size, outline, and brand-coloured icon (`BrainCircuitRegular`) as the others. Giving it a destination later takes two steps and needs no restyling: add its URL to `Credpl_Admin_Manager::enqueue_assets()`'s localized `urls` (and to the config interface), then set the card's `urlKey`.
- **Navigation**: clicking anywhere on a navigation card that has a destination goes to it (`window.location.assign()`). Since Fluent's `Card` renders a plain `<div>`, each card's title is *also* a real Fluent `Link` (`<a href>`), which is what gives keyboard focus/Enter activation, screen-reader "link" semantics, and ctrl/cmd/middle-click "open in new tab"; the card-level click handler skips clicks that originate on that anchor so navigation never fires twice. `focusMode="off"` stops the `Card` itself from being a second tab stop beside the link.
- **PHP→React config**: `Credpl_Admin_Manager::enqueue_assets()` localizes `window.credplCredentialsManagerPage = { urls: { blocks, credentials, certifications, exams, integrations } }` — plain `admin.php?page=…` URLs, no nonce (navigation only, no state change). Labels, icons, and the section grouping stay on the React side (labels through `__()`, text domain `credentials-manager-plugin`), so moving a card between sections is a TSX-only change.

### 6.9 Update checking (GitHub Releases)

The plugin isn't on WordPress.org, so WordPress can't learn about new versions from there. `Credpl_Updater` (`includes/class-credpl-updater.php`, §10 v76) makes WordPress notice and install new releases published on this repository's GitHub Releases page instead, through core's own update system — the normal "update available" notice, one-click update, and per-plugin auto-update toggle.

- **Mechanism.** The plugin header carries `Update URI: https://github.com/aidevme/aidevme-blog-wordpress-plugins`. Since WordPress 5.8, core then skips WordPress.org for this plugin and, from `wp_update_plugins()`, applies the `update_plugins_{$hostname}` filter — here `update_plugins_github.com`. `Credpl_Updater::filter_update()` answers it with the newest matching release (`slug`, `version`, `url`, `package`); **core** compares that `version` with the installed one and files it under "update available" or "no update" (verified against `wp-includes/update.php`), so the class always reports the latest release rather than deciding itself. The header also stops WordPress.org from offering an unrelated plugin that happens to share this slug.
- **What counts as a release.** Only *published GitHub Releases* — never the `main` branch, so merging a version bump offers nothing until the **Release Credentials Manager Plugin** workflow (§10 v74/v75) has run. A release qualifies when it is **not a draft**, is tagged `credentials-manager-plugin-v<X.Y.Z>` (plugin-scoped, because this repository will hold more than one plugin, so a bare `v1.2.3` or another plugin's tag is ignored), and has the `credentials-manager-plugin-<X.Y.Z>.zip` asset attached and fully uploaded. The newest is chosen by `version_compare()` on the version, not by the order GitHub lists releases, so `0.0.100` beats `0.0.83`. `Credpl_Updater::select_release()` is a pure function of a decoded GitHub response, which is what makes this testable without WordPress.
- **Pre-releases.** Included while the installed version is below `1.0.0`, excluded from `1.0.0` on (`Credpl_Updater::include_prereleases()`), overridable with the `credpl_updater_include_prereleases` filter. This default exists because the Release workflow marks every `0.x` release a pre-release; the library most plugins use for this (Plugin Update Checker) always ignores pre-releases, which would have meant no update was ever offered.
- **Why not Plugin Update Checker.** It derives a release's version by stripping a leading `v` from the tag, so this repository's plugin-scoped tags would be read as the "version" `credentials-manager-plugin-v0.0.83`. Rather than give up plugin-scoped tags, this plugin has its own small updater and no bundled library.
- **The package URL and details URL are built, not copied.** `select_release()` constructs `https://github.com/<repo>/releases/download/<tag>/<asset name>` (and the release-page URL) from the repository, tag and asset name; nothing in the API response is used as a URL, so a malformed or tampered response can't point WordPress at another host. The zip's top-level folder is `credentials-manager-plugin/`, so WordPress replaces the installed plugin in place. (GitHub's automatic "Source code" zip has a different folder name and is never used.)
- **Requests and caching.** One `wp_remote_get()` to `https://api.github.com/repos/<repo>/releases?per_page=100` (100 is the API maximum — this plugin's release has to be found among every plugin's releases), 8-second timeout, with an `Accept`, `X-GitHub-Api-Version`, and `User-Agent: credentials-manager-plugin/<version>` header (GitHub rejects requests with no User-Agent; the plugin's own replaces WordPress's default, which includes the site URL). The result — including "no release" — is cached in the `credpl_latest_release` site transient for **6 hours**; a failed request (network error, non-200 status, unparseable body) is cached for **1 hour**, so an outage or a rate limit never adds a slow request to every admin page. Unauthenticated GitHub API calls are limited to 60 per hour per IP, which this comfortably stays under. The plugin also clears its cache whenever core clears its own plugin-update cache (`delete_site_transient_update_plugins`, fired by `wp_clean_plugins_cache()`, which core calls after a plugin is installed or updated from the admin). **Known limitation (§10 v79):** **Dashboard → Updates → Check Again** does *not* do that — on that page core only shortens WordPress's own freshness window to one minute and re-applies the update filter, which this plugin answers from its cache — so a release published within the last 6 hours may not appear there. (Reproduced against the released code after 0.0.85 was published; an earlier version of this section wrongly said Check Again cleared the cache.) To force it: `wp eval 'delete_site_transient("credpl_latest_release"); delete_site_transient("update_plugins");'`.
- **Failure is silent.** Any failure returns "no update" to WordPress; nothing is shown to the user and nothing can break the admin.
- **"View version x details".** `Credpl_Updater::filter_plugins_api()` answers `plugins_api` for this slug with the release's version, download link, publication date, an author link to `https://aidevme.com` (matching the header's `Author URI`), the `Requires at least`/`Requires PHP` values from the plugin header, and the release notes as the changelog tab — escaped (`esc_html()` then `wpautop()`), since they are untrusted text. Without it WordPress would ask WordPress.org about the slug and show "Plugin not found".
- **Limits.** The repository must be **public** (there is no token support; a private repository would need authenticated asset downloads). The plugin's newest release must be among the repository's latest 100. Updates are only offered once a site runs a version that contains this class (**0.0.84** and later): sites on older versions have to be updated by hand once.
- **Trust.** Anyone who can publish a release on this repository can deliver code to every site running the plugin, and WordPress does not verify the zip beyond downloading it over HTTPS. Keep two-factor authentication on for the accounts that can publish, and keep releasing from `main` only (§10 v75).

### 6.10 Skills screen

The screen for the `skills` table (§4.6), added in §10 v81. Same list + add/edit pair as Credential Blocks (§6.3) and Microsoft Exams (§6.5) — both halves are React apps on Fluent UI 9, with PHP doing only the page chrome, the data localization, and the save/delete handlers (`Credpl_Admin_Skills`, `includes/class-credpl-admin-skills.php`).

- **Pages**: `PAGE_SKILLS` (`credpl-skills`, "All Skills", visible in the submenu) and `PAGE_SKILL_NEW` (`credpl-skill-new`, "Add Skill" / "Edit Skill" — registered normally but CSS-hidden from the sidebar like the other four "Add …" screens, §6.1). All Skills is where the landing page's **Skills** card goes (§6.8).
- **List** (`src/skills-list.tsx`, the tenth webpack entry, `build/skills-list.js`): a Fluent UI 9 plain `Table` — the same building blocks and the same selection / toolbar / delete-confirmation / footer behavior as the other list screens (§6.2.3). Columns: **Skill Name** (a `Link` to the edit screen) and **Description** (truncated to one line with the full text in a `title`) — the GUID is deliberately not shown (§10 v82). Both are sortable, by reloading to `?orderby=…&order=…`; the column IDs are exactly two of `Credpl_Data::get_skills()`'s allowlist entries (`skill_name`, `description`), defaulting to `skill_name` A–Z. The toolbar is **Back** (to the landing page, §6.8), **New**, **Edit** (exactly one row selected) and **Delete** (one or more) — no Sync button, since skills aren't filled from an external catalog. `Credpl_Admin_Skills::enqueue_list_assets()` runs `Credpl_Data::get_skills()` once and localizes `window.credplSkillsList = { rows, listUrl, orderby, order, backUrl, addNewUrl, bulkDeleteUrl, noItemsText }`, each row `{ id, skillName, description, editUrl }`. Styles: `src/styles/skillsList.styles.ts` (`useSkillsListStyles`).
- **Add/Edit form** (`src/skill.tsx`, the eleventh webpack entry, `build/skill.js`): a plain HTML form POSTing to `admin-post.php?action=credpl_save_skill`, with a Fluent UI `Input` for **Skill Name** (required) and a `Textarea` for **Description**. The **GUID** is not shown or submitted (§10 v82) — it is assigned by `Credpl_Data::insert_skill()` on first save and `update_skill()` ignores any `guid` it is given (§4.6). Data flow: `Credpl_Admin_Skills::enqueue_edit_form_assets()` localizes `window.credplSkillForm = { id, nonce, actionUrl, skillName, description, listUrl }`. Styles: `src/styles/skill.styles.ts` (`useSkillStyles`).
- **Save** (`Credpl_Admin_Skills::save()`): `manage_options` check, `check_admin_referer( 'credpl_save_skill' )`, then `skill_name` through `sanitize_text_field()` and `description` through `sanitize_textarea_field()`. An empty name is rejected server-side (the form's `required` attribute is only the first line of defense) by redirecting back with `error=missing_name`; a database failure redirects back with `error=save_failed`. Success redirects to the edit screen with `updated=1`, which shows a "Skill saved." notice and a **Back to All Skills** button.
- **Delete**: one handler, `Credpl_Admin_Skills::bulk_delete()` (`admin-post.php?action=credpl_bulk_delete_skills&ids[]=…`), driven by the toolbar's Delete button after a `ConfirmationDialog`. `manage_options` check plus `check_admin_referer()` against one fixed nonce action (the selected IDs are only known client-side at click time). There is no separate single-row delete action — a single delete is a bulk delete of one. Redirects with `bulk_deleted=N`.
...
3. Output the default template (§10 v84): an optional heading (the block's own `title`, per the table above), then one **card** per credential, in a light panel:

   - **Badge** — the credential's badge image in a round, pale-blue disc, linking to `credential_link` if set. A credential with no badge image gets a generic blue star badge instead (inline SVG), so its card still lines up with the others.
   - **Title** — bold, linking to `credential_link` (new tab, `rel="noopener noreferrer"`) if set.
   - **Pill** under the title — `"{issuer} {credentials_type}"`, e.g. `Microsoft Applied Skills` (`Credpl_Shortcode::format_tag()`; uppercased by the stylesheet, not stored that way). Either half is dropped if empty and the pill is omitted if both are. **This deliberately renders `issuer` again** (§10 v12 had removed it from this output); `credentials_type` is shown on every credential type, including Awards.
   - **Dates column** on the right, behind a vertical divider — for any credential whose `credentials_type` is **not** `Awards`, an **"Earned on"** entry and, below it, an **"Expires on"** entry, each with a calendar icon, a small label, and the date in a `<time datetime="Y-m-d">` element formatted via `date_i18n()` against the site's `date_format` option (`Credpl_Shortcode::render_date()`). Either entry is dropped if that date isn't set, and the whole column is omitted if neither is. (Before v84 this was one combined "Expires on … · Earned on …" line.)
   - For a credential whose `credentials_type` is `Awards` (§10 v31), the dates column is suppressed instead (§10 v32 — those dates aren't meaningful for an award) and two lines are appended under the pill — **"Award Category: …"** and **"Technology Area: …"** — each only when that field is actually set (empty ones are skipped); credentials of any other type never render these two lines, even if `award_category`/`technology_area` hold a leftover value (§10 v30 covers how that can happen on the form side).

   ```html
   <div class="credpl-credential-block" data-credpl-block="a2949a2">
     <h3 class="credpl-credential-block-heading">Applied Skill(s)</h3>
     <ul class="credpl-credential-list">
       <li class="credpl-credential-item">
         <a class="credpl-credential-badge" href="https://…" target="_blank" rel="noopener noreferrer" aria-label="Microsoft Applied Skills: …">
           <img src="https://…" alt="Microsoft Applied Skills: …" />
         </a>
         <div class="credpl-credential-info">
           <h3 class="credpl-credential-title"><a href="https://…" target="_blank" rel="noopener noreferrer">Microsoft Applied Skills: …</a></h3>
           <span class="credpl-credential-tag">Microsoft Applied Skills</span>
           <!-- Awards only: <p class="credpl-credential-meta">Award Category: …</p> -->
         </div>
         <div class="credpl-credential-dates">
           <div class="credpl-credential-date">
             <svg class="credpl-credential-date-icon" …></svg>
             <span class="credpl-credential-date-text">
               <span class="credpl-credential-date-label">Earned on</span>
               <time class="credpl-credential-date-value" datetime="2026-01-21">January 21, 2026</time>
             </span>
           </div>
           <!-- a second .credpl-credential-date for "Expires on", when set -->
         </div>
       </li>
       <!-- … -->
     </ul>
...
4. `assets/css/credpl-credential-block.css` styles that markup (§10 v84): the block is a light panel (`--credpl-panel-bg`, rounded) under a heading with a short blue accent bar (`::before`); each `.credpl-credential-item` is a white, rounded, 1px-bordered flex row. **Type sizes are fixed pixel values held in custom properties** (§10 v86): the block heading is `--credpl-heading-size` (20px); the credential title is `--credpl-title-size` (20px, §10 v87); the date values and the Award Category/Technology Area lines are `--credpl-text-size` (18px); and the pill and the "Earned on" / "Expires on" labels are `--credpl-small-size` (13px). `.credpl-credential-block` itself is set to `--credpl-text-size`, so every `em`-based measurement inside it (padding, gaps, the calendar icon, the `14.5em` dates column) is anchored to 18px instead of the theme's body size. The badge disc is 80px (56px on narrow screens), the title is bold, the pill is a small rounded, light-blue, uppercase label, and the dates column is a fixed `14.5em` wide so the dividers line up from card to card. Below `640px` the row wraps: the dates drop under the badge/title, separated by a top border, and lay out side by side. **Hover** (§10 v85): on `:hover` and `:focus-within` a card lifts 3px (`translateY( -3px )`), its border turns `--credpl-accent`, and it gains `--credpl-hover-shadow`, animated over 0.2s; under `prefers-reduced-motion: reduce` the animation and the lift are dropped (the border/shadow change remains). It applies to every card, whether or not the credential has a link — the cards are display cards, not a single click target. **Colours are custom properties on `.credpl-credential-block`** — as are the four sizes above — (`--credpl-accent`, `--credpl-accent-strong`, `--credpl-tag-bg`, `--credpl-disc-bg`, `--credpl-card-bg`, `--credpl-card-border`, `--credpl-text`, `--credpl-muted`, `--credpl-icon`, `--credpl-panel-bg`, `--credpl-hover-shadow`), so a theme restyles it by overriding a variable rather than fighting selectors; every rule is otherwise kept low-specificity, and the font is inherited from the theme (no web font is loaded). Enqueued only on pages whose content actually contains the shortcode (`has_shortcode( $post->post_content, 'credential-block' )`), not site-wide. `.credpl-credential-list`/`.credpl-credential-item` are the one exception to "kept low-specificity": both carry `list-style: none !important` (plus a belt-and-suspenders `.credpl-credential-item::marker { content: none; }`), since without it a theme's own content-area list styling can reintroduce a bullet in front of each entry — and, because `.credpl-credential-item` is `display: flex`, that reintroduced marker renders on its own line above the badge/title row rather than beside it (§10 v33). Apart from that, the only other `!important`s in the stylesheet are on the `font-size` of the heading and the credential title (§10 v87) — both are `<h3>`s, which themes routinely size with tag-based rules more specific than one class, so a plain declaration lost; a theme that wants a different size sets `--credpl-heading-size` / `--credpl-title-size` rather than out-specifying them.
...
credentials-manager-plugin/
  ...
  includes/
    ...
    class-credpl-admin-menu.php         (top-level + 10 submenu registration, 4 CSS-hidden via admin_head, §6.1)
    class-credpl-admin-manager.php      (Credentials Manager landing page callback + asset enqueue, §6.8 — renders a React mount point, localizes the navigation cards' destination URLs)
    ...
    class-credpl-admin-blocks.php       (Credential Blocks list/add/edit page callbacks + save/delete, §6.3 — the list page callback renders a React mount point, not a WP_List_Table, §6.3.2/§10 v59)
    class-credpl-admin-ms-certifications.php  (Microsoft Certifications list/add/edit page callbacks + save/delete/sync, §6.4 — the list page callback renders a React mount point, not a WP_List_Table, §6.4.2/§10 v60)
    class-credpl-admin-ms-exams.php     (Microsoft Exams list/add/edit page callbacks + save/delete/sync, §6.5 — the list page callback renders a React mount point, not a WP_List_Table, §6.5.2/§10 v61)
    class-credpl-shortcode.php          ([credential-block] registration + rendering, §7)
    class-credpl-updater.php            (Credpl_Updater — offers new GitHub releases to WordPress's own update system, §6.9)
  ...
  bin/
    lint-php.js                         (`npm run lint-php` — cross-platform Node script running `php -l` over every .php file in the plugin, §6.2.1/§10 v64; not shipped in dist/, same as tsconfig.json/webpack.config.js — see DEVELOPER.md)
    bump-version.js                     (`prebuild` lifecycle hook — auto-increments the patch version in package.json + credentials-manager-plugin.php before every `npm run build`, §6.2.1/§10 v66; not shipped in dist/, same as lint-php.js)
    package-zip.js                       (`postbuild` lifecycle hook + standalone `npm run package-zip` — rebuilds dist/credentials-manager-plugin.zip from includes/, build/, assets/, and the fixed set of shippable root docs, using adm-zip, §6.2.1/§10 v67; not shipped in dist/, same as bump-version.js — replaces the manual PowerShell staging script this plugin used before this file existed)
  src/
    ...
    credentials-blocks-list.tsx         (TypeScript + JSX source for the React Credential Blocks list — Fluent UI 9 Table building blocks, §6.3.2)
    ms-certification.tsx                (TypeScript + JSX source for the React Add/Edit Microsoft Certification form, §6.4)
    ms-certifications-list.tsx          (TypeScript + JSX source for the React Microsoft Certifications list — Fluent UI 9 Table building blocks, §6.4.2)
    ms-exam.tsx                         (TypeScript + JSX source for the React Add/Edit Microsoft Exam form, §6.5)
    ms-exams-list.tsx                   (TypeScript + JSX source for the React Microsoft Exams list — Fluent UI 9 Table building blocks, §6.5.2)
    skill.tsx                           (TypeScript + JSX source for the React Add/Edit Skill form, §6.10)
    skills-list.tsx                     (TypeScript + JSX source for the React Skills list — Fluent UI 9 Table building blocks, §6.10)
    credentials-manager-page.tsx        (TypeScript + JSX source for the React Credentials Manager landing page — four section `Card`s of navigation `Card`s, §6.8)
    tools/
      index.ts                             (barrel export — re-exports everything from listUrls.ts below, §10; every list screen imports from './tools' rather than './tools/listUrls' directly, same barrel convention as components/ and styles/)
      listUrls.ts                          (EM_DASH, getCurrentSortDirection()/buildSortUrl()/buildNextSortUrl()/buildBulkDeleteUrl() — shared by all four list screens, extracted from four near-identical hand-written copies, §10; REACT-DEVELOPER-GUIDE.md §4 — every list screen imports from the tools/ barrel rather than redefining any of these)
    components/
      index.ts                             (barrel export — re-exports every component/prop-type below, §10 v51; credentials-list.tsx and any future consumer import from './components' rather than each component's own file path)
      dialogs/
      ...
      tables/
        table.tsx                           (DataTable + the generic ColumnDef<TRow> — the shared Fluent UI 9 list table every list screen renders through: selection checkboxes, sortable headers, empty state, footer; §6.2.3/§10 v83)
        use-row-selection.ts                (useRowSelection() — the selectedIds Set + derived singleSelectedRow every list screen's toolbar reads, §10 v83)
        table-footer.tsx                    (TableFooter — generic Fluent UI 9 table footer, not Credentials-specific; first used below credentials-list.tsx's table, §6.2.3/§10 v49)
    styles/
      index.ts                             (barrel export — re-exports every `use*Styles` hook below, §10 v52; every consumer imports from './styles' rather than each hook's own file path)
      ...
      credentialsManagerPage.styles.ts    (useCredentialsManagerPageStyles, §6.8)
      credentialsBlock.styles.ts          (useCredentialsBlockStyles — extracted from credentials-block.tsx)
      credentialsBlocksList.styles.ts     (useCredentialsBlocksListStyles, §6.3.2)
      msCertification.styles.ts           (useMsCertificationStyles, §6.4)
      msCertificationsList.styles.ts      (useMsCertificationsListStyles, §6.4.2)
      msExam.styles.ts                    (useMsExamStyles, §6.5)
      msExamsList.styles.ts               (useMsExamsListStyles, §6.5.2)
      table.styles.ts                     (useTableStyles — DataTable's tableWrap/noItems rules, moved out of each list's own styles file, §10 v83)
      tableFooter.style.ts                (useTableFooterStyles — extracted from table-footer.tsx, §10 v50; note the singular ".style.ts", unlike every other file in this directory's plural ".styles.ts" — an inconsistency introduced by this filename being requested explicitly rather than following the established pattern)
      toolbarCard.styles.ts               (useToolbarCardStyles — extracted from credentialsList.styles.ts, §6.2.3/§10 v55)
  build/                               (compiled by `npm run build` — shipped, not gitignored)
    ...
    credentials-blocks-list.js
    credentials-blocks-list.asset.php
    ms-certification.js
    ms-certification.asset.php
    ms-certifications-list.js
    ms-certifications-list.asset.php
    ms-exam.js
    ms-exam.asset.php
    ms-exams-list.js
    ms-exams-list.asset.php
    skill.js
    skill.asset.php
    skills-list.js
    skills-list.asset.php
    credentials-manager-page.js
    credentials-manager-page.asset.php
  assets/
    css/
...
- Activating the plugin creates `{$wpdb->prefix}credentials`, `{$wpdb->prefix}credential_blocks`, `{$wpdb->prefix}microsoft_certifications`, `{$wpdb->prefix}microsoft_exams`, and `{$wpdb->prefix}skills`, and sets `credpl_db_version`.
...
- A top-level **"Credentials Manager"** admin menu item appears, with **Credentials Manager**, **All Credential Blocks**, **All Credentials**, **All Microsoft Certifications**, **All Microsoft Exams**, **All Skills**, and **Integration** submenus visible (the five "Add …" screens are intentionally not in this list, §6.1, §10 v26), and clicking the top-level item lands on the same screen as the **Credentials Manager** submenu — a page with the heading and four full-width Fluent UI 9 section cards — **Main** (Credential Blocks, Credentials), **Miscellaneous** (Microsoft Certifications, Microsoft Exams, Skills), **Style** (empty, shows "Nothing here yet."), and **Integrations** (Integrations) — with no console errors (§6.8, §10 v72–v75, v77). Hovering a navigation card lifts it and adds a shadow/brand border (the section cards themselves have no hover effect); clicking anywhere on a navigation card (or activating its title link from the keyboard) opens the matching screen — the "All …" lists, or Integration — and ctrl/cmd-clicking the title opens it in a new tab. The **Skills** card behaves like the others and opens **All Skills** (§6.10, §10 v81).
...
- On the Credentials list, checking a row's checkbox selects it (visually highlighted, checkbox shows checked); checking the header checkbox selects every row, and shows a `"mixed"` indeterminate state when only some rows are checked. The toolbar's four buttons (Back, New, Edit, Delete) show only an icon, no text — hovering (or focusing) any of them, including while disabled, shows a tooltip explaining what it does and when it's enabled. **Back** is always enabled and returns to the Credentials Manager page (§6.8, §10 v74) — this applies to the Credential Blocks, Microsoft Certifications, and Microsoft Exams lists' toolbars too. With nothing selected, **New** is enabled and **Edit**/**Delete** are disabled; with exactly one row selected, **Edit** is also enabled and navigates to that credential's edit screen when clicked; with one or more rows selected, **New** is disabled and **Delete** is enabled. Below the table, a footer shows "Number of Records: N" (the total shown) and "Number of Selected Records: N" on the same line, both updating live as rows are checked/unchecked.
...
- On the Credential Blocks list, clicking the Title or Description column header sorts the list ascending, clicking it again sorts descending, and the arrow indicator reflects the current sort — the same behavior as the already-sortable columns on the other three list tables. This list also renders as a Fluent UI `Table` (§6.3.2, §10 v59) with the same selection/toolbar/delete-confirmation/footer behavior Credentials has (§6.2.3): checking rows enables the toolbar's Edit (exactly one selected) and Delete (one or more) buttons, New is enabled only with nothing selected, and clicking a shortcode field selects its text for copying.
- Pasting that shortcode into a page or post's content renders a heading (the block's own Title field, when set — not overridable per shortcode instance) with a short blue bar above it, followed by one white, rounded card per selected credential (§7, §10 v84): a round badge (the badge image, or a generic star if none), the title (linking to `credential_link` when set), a pill reading "{issuer} {credentials type}" (e.g. "MICROSOFT APPLIED SKILLS"), and — for any credential whose Credentials Type is not `Awards` — a divided right-hand column with an "Earned on" and/or "Expires on" entry for whichever of those dates are set. The dividers line up across cards, and below 640px the dates move under the title. The block heading and the credential title are 20px, the date values and award lines 18px, and the pill and date labels 13px, regardless of the theme's own body text size or its `<h3>` rules (§10 v86, v87). Hovering (or keyboard-focusing a link inside) a card lifts it slightly, adds a shadow and turns its border blue (§10 v85). Styled by `credpl-credential-block.css`. Renaming the block in the admin screen updates the heading everywhere the shortcode is pasted.
- A Credential Block containing an `Awards`-type credential with both Award Category and Technology Area set renders "Award Category: …" and "Technology Area: …" lines for that credential on the front end instead of the dates column, even if `earned_on`/`expires_on` are set on it (§10 v32); a credential of any other Credentials Type never shows either Award/Technology line, and an `Awards`-type credential with just one of the two fields set shows only that one line.
...
- The Microsoft Certifications list renders as a Fluent UI `Table` (§6.4.2, §10 v60), showing every existing certification with the same selection/toolbar/delete-confirmation/footer behavior Credentials has (§6.2.3): checking rows enables the toolbar's Edit (exactly one selected) and Delete (one or more) buttons, New is enabled only with nothing selected. Clicking the Title, Certification Type, Type, or Last Modified column header reloads the page sorted by that column (ascending, then descending on a second click).
- Clicking the toolbar's **Sync Certifications** button on the Microsoft Certifications list opens a confirmation dialog; confirming fetches Microsoft Learn's catalog, creates a row for every certification `uid` not already present, updates every row whose `uid` already exists, and shows a "Sync complete: N created, M updated, K skipped." notice with accurate counts — run it twice in a row and the second run reports 0 created (everything already exists) rather than duplicating rows. If anything was skipped, the notice also names a reason (not just a count). Cancelling the dialog performs no sync.
- The Skills list renders as a Fluent UI `Table` (§6.10, §10 v81) with Skill Name and Description columns (no GUID column), and the same selection/toolbar/delete-confirmation/footer behavior as the other lists. Clicking either column header reloads the page sorted by it; with no skills it shows "No skills yet.".
- Creating a Skill via "Add Skill" (Skill Name, Description) saves a new row with a server-generated GUID (a 36-character UUID v4) and appears in the Skills list. Editing it pre-fills both fields, with no GUID field on the form, and leaves the stored GUID unchanged after saving. Submitting an empty Skill Name is rejected with an error notice. Selecting one or more skills and confirming Delete removes them.
...
- The Microsoft Exams list renders as a Fluent UI `Table` (§6.5.2, §10 v61), with the same selection/toolbar/delete-confirmation/footer behavior as the other three lists. Clicking the Title, Display Name, Type, or Last Modified column header reloads the page sorted by that column.
- Clicking the toolbar's **Sync Exams** button on the Microsoft Exams list opens a confirmation dialog; confirming fetches Microsoft Learn's catalog, creates a row for every exam `uid` not already present, updates every row whose `uid` already exists, and shows a "Sync complete: N created, M updated, K skipped." notice (with a "First reason: …" detail whenever anything was skipped) — run it twice in a row and the second run reports 0 created. Cancelling the dialog performs no sync.
- Deactivating or uninstalling the plugin leaves all five tables and all rows untouched.
- **Update notices (§6.9).** On a site running this version or later, publishing a newer non-draft `credentials-manager-plugin-v<X.Y.Z>` GitHub Release (with its `credentials-manager-plugin-<X.Y.Z>.zip` asset) makes the plugin's row on the Plugins screen show "There is a new version of Credentials Manager available" after WordPress's next update check that runs once the plugin's own GitHub cache (up to 6 hours, §6.9) has expired or been cleared — **Check Again** alone does not clear it — and the update installs in place from that zip. A release of another plugin in the same repository, a draft, a release with no matching asset, and a merge to `main` with no release, all produce no notice. While the installed version is below `1.0.0`, pre-releases are offered; from `1.0.0` they are not. If GitHub can't be reached, no error is shown anywhere.
