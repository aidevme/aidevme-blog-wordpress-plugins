...
The top-level menu item's own click target (§6.1, §10 v72–v75), page slug `Credpl_Admin_Menu::PAGE_MANAGER` (`credpl-credentials-manager`). A React app on Fluent UI 9, built like the list/form screens (§6.2.1): four full-width **section cards**, each holding the clickable **navigation cards** for that group.
...
- **Sections and their cards** (grouping is defined in the TSX's `sections` array; each section card is full width, with an `<h2>` title via a `CardHeader`, and is a plain container — not clickable, no hover effect):
  ...
  | **Miscellaneous** | Microsoft Certifications → All Microsoft Certifications (`PAGE_MS_CERTIFICATIONS`); Microsoft Exams → All Microsoft Exams (`PAGE_MS_EXAMS`) |
...
- A top-level **"Credentials Manager"** admin menu item appears, with **Credentials Manager**, **All Credential Blocks**, **All Credentials**, **All Microsoft Certifications**, **All Microsoft Exams**, and **Integration** submenus visible (the four "Add …" screens are intentionally not in this list, §6.1, §10 v26), and clicking the top-level item lands on the same screen as the **Credentials Manager** submenu — a page with the heading and four full-width Fluent UI 9 section cards — **Main** (Credential Blocks, Credentials), **Miscellaneous** (Microsoft Certifications, Microsoft Exams), **Style** (empty, shows "Nothing here yet."), and **Integrations** (Integrations) — with no console errors (§6.8, §10 v72–v75). Hovering a navigation card lifts it and adds a shadow/brand border (the section cards themselves have no hover effect); clicking anywhere on a navigation card (or activating its title link from the keyboard) opens the matching screen — the "All …" lists, or Integration — and ctrl/cmd-clicking the title opens it in a new tab.
