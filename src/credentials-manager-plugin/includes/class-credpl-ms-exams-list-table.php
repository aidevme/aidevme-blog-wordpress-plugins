<?php
/**
 * List table for the Microsoft Exams admin screen. See
...
class Credpl_Ms_Exams_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		return array(
			...
			'display_name'  => __( 'Display Name', 'credentials-manager-plugin' ),
	...
	/**
	 * Same rationale as Credpl_Ms_Certifications_List_Table::get_sortable_columns()
	 * — `orderby` values pass straight through to Credpl_Data::get_ms_exams(),
	 * which validates them against its own allowlist. Icon isn't
	 * meaningful to sort by, so it's left out.
	 */
	...
	public function prepare_items() {
		...
		$this->items = Credpl_Data::get_ms_exams(
			array(
				'orderby' => $orderby,
				'order'   => $order,
			)
	...
	/**
	 * `last_modified` is a full DATETIME — same formatting as
	 * Credpl_Ms_Certifications_List_Table::format_last_modified().
	 */
	...
	public function column_title( $item ) {
		...
		$delete_url = wp_nonce_url(
			add_query_arg(
				array(
					'action' => 'credpl_delete_ms_exam',
					'id'     => absint( $item['id'] ),
				),
				admin_url( 'admin-post.php' )
			),
			'credpl_delete_ms_exam_' . absint( $item['id'] )
		);
		...
		$actions = array(
			...
			'delete' => sprintf(
				...
				esc_url( $delete_url ),
				esc_js( __( 'Delete this Microsoft Exam?', 'credentials-manager-plugin' ) ),
	...
	public function no_items() {
		esc_html_e( 'No Microsoft Exams yet.', 'credentials-manager-plugin' );
	}
}
