...
interface ColumnDef {
	...
	renderCell: ( row: MsCertificationRow ) => ReactNode;
}
	...
	return (
		<>
			...
			<div className={ styles.tableWrap }>
				<Table aria-label={ __( 'Microsoft Certifications', 'credentials-manager-plugin' ) }>
					<TableHeader>
						<TableRow>
							<Tooltip
								content={ __( 'Select or deselect every Microsoft Certification currently shown in the list.', 'credentials-manager-plugin' ) }
								relationship="label"
								withArrow
							>
								<TableSelectionCell
									...
									checkboxIndicator={ { 'aria-label': __( 'Select all Microsoft Certifications', 'credentials-manager-plugin' ) } }
								/>
							</Tooltip>
							{ columns.map( ( column ) => (
							...
							return (
								<TableRow key={ row.id } appearance={ selected ? 'brand' : 'none' } aria-selected={ selected }>
									<TableSelectionCell
										...
										checkboxIndicator={ { 'aria-label': __( 'Select Microsoft Certification', 'credentials-manager-plugin' ) } }
									/>
									{ columns.map( ( column ) => (
